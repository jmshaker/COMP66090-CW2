############################################################################################################################################
#   Modified from Jansen and Kuhl's original: https://github.com/TomJansen25/Extracting-Core-Claims/blob/master/AIDA_Check_and_Rewrite.py  #
############################################################################################################################################
...
import json
#import sys
#sys.path.append('../pyStatParser-master')
...
def check_if_atomic(sentence, sentence_lower, parsed_sentence, tags):
    counter = 0
...
def check_if_independent(sentence, sentence_lower):
...
def check_if_absolute(sentence, sentence_lower, tags):
    ...
    if searchObj != None:
        ...
        return False
    elif (("MD" in tags) or ("VBD" in tags)):
        return False
    else:
        return True
...
def perform_AIDA_check(sentence, sentence_lower, parsed, tags):
    if check_if_atomic(sentence, sentence_lower, parsed, tags):
        Atomic = True
    else:
        Atomic = False
    if check_if_independent(sentence, sentence_lower):
        Independent = True
    else:
        Independent = False
...
if __name__ == "__main__":
    ...
    #jsonFile = open('../all-all-claims-AIDA.json', 'w')
    ...
    extracted_sentences = open('../all-all-claims.json').readlines()
    for line in extracted_sentences:
        obj = json.loads(line)
        lines.append(line)
    ...
    for sentence in sentences:
        jsonFile = open('../all-all-claims-AIDA.json', 'a')
        ...
        Atomic, Independent, Declarative, Absolute = perform_AIDA_check(sentence, sentence_lower, parsed, tags)
        
        if Atomic == True and Independent == True and Declarative == True and Absolute == True:
            AIDA = True
            rewritten_sentence = sentence
            print("AIDA - " + sentence)
            jsonFile.write(lines[i])
            jsonFile.close()
